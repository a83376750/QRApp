// QR_Encode.cpp : CQR_Encode 类的执行文件
...
/////////////////////////////////////////////////////////////////////////////
// QR码的版本(型号)的信息
static QR_VERSIONINFO QR_VersonInfo[] = {{0}, // (假如为:Ver.0)
										 { 1, // Ver.1
										    26,   19,   16,   13,    9,
										   0,   0,   0,   0,   0,   0,   0,
										   1,  26,  19,
...
// 用 途：对数据进行编码
// 参 数：纠错等级、版本(型号)(0=自动)、版本(型号)自动扩张标志、掩模号码(-1=自动)、进行编码的数据、进行编码的数据长度
// 备 注：当编码成功=TRUE、无可用数据、或者容量超出时=FALSE
...
BOOL CQR_Encode::EncodeSourceData(LPCSTR lpsSource, int ncLength, int nVerGroup)
{
	...
	/////////////////////////////////////////////////////////////////////////
	// 连接字母数字模块与数字模块的排列结合？？
	int ncSrcBits, ncDstBits; //单位字母数字模块的 位长度？？
	int nBlock = 0;

	while (nBlock < m_ncDataBlock - 1)
	{
		int ncJoinFront, ncJoinBehind; //前和后的8位字节模块的字
		int nJoinPosition = 0; // 8位字节模块的结合：-1=结合之前、0=没有结合、1=结合之后
        // "数字-字母数字"与"字母数字-数字"的排列
	...
	/////////////////////////////////////////////////////////////////////////
    //位序列化
	int ncComplete = 0; // 处理完毕的数据计数器
	WORD wBinCode;

	m_ncDataCodeWordBit = 0; //几位单位处理计数器
...
// 用  途：获取位长度
// 参  数：类型的数据模式、数据长度、版本（型号）组
// 返回值：数据位长度
// 备  注：在汉字模式的数据长度参数是字符数而不是字节数
...
// 用  途：设置位流
// 参  数：插入位置、阵列数据位，数据位(最大16位)
// 返回值：下一个插入位置(缓冲区溢出是=-1)
// 备  注：m_byDataCodeWord结束后设置(需要初始化)
...
void CQR_Encode::GetRSCodeWord(LPBYTE lpbyRSWork, int ncDataCodeWord, int ncRSCodeWord)
{
	...
	for (i = 0; i < ncDataCodeWord ; ++i)
	{
		if (lpbyRSWork[0] != 0)
		{
			BYTE nExpFirst = byIntToExp[lpbyRSWork[0]]; //算出第一项系数

			for (j = 0; j < ncRSCodeWord; ++j)
			{
				// 各项乘数第一项中（乘数加255→α^ 255 = 1）
...
void CQR_Encode::FormatModule()
{
	...
	if (m_nMaskingNo == -1)
	{
		...
		SetMaskingPattern(m_nMaskingNo); // 掩蔽
		SetFormatInfoPattern(m_nMaskingNo); // 图形布局格式信息
		int nMinPenalty = CountPenalty();

		for (i = 1; i <= 7; ++i)
		{
			SetMaskingPattern(i); // 掩蔽
			SetFormatInfoPattern(i); // 图形布局格式信息
...
//用途：设置功能块模/ /预备考：格式信息功能模块登记只（实数据空白）
...
void CQR_Encode::SetCodeWordPattern()
{
	...
	int nCoef_x = 1; // x轴方向配置
	int nCoef_y = 1; // y轴方向配置
	...
	for (i = 0; i < m_ncAllCodeWord; ++i)
	{
		for (j = 0; j < 8; ++j)
		{
			...
			while (m_byModuleData[x][y] & 0x20); //功能模块除外
...
void CQR_Encode::SetMaskingPattern(int nPatternNo)
{
	...
	for (i = 0; i < m_nSymbleSize; ++i)
	{
		for (j = 0; j < m_nSymbleSize; ++j)
		{
			if (! (m_byModuleData[j][i] & 0x20)) // 功能模块除外
			{
				BOOL bMask;
...
int CQR_Encode::CountPenalty()
{
	...
	// 同一列中的1 : 1 : 3 : 1 :一比率（黑暗:明:黑暗:明:黑暗）的图形
	for (i = 0; i < m_nSymbleSize; ++i)
	{
	...
	// 同一行中的1 : 1 : 3 : 1 :一比率（黑暗:明:黑暗:明:黑暗）的图形
	for (i = 0; i < m_nSymbleSize; ++i)
	{
		for (j = 0; j < m_nSymbleSize - 6; ++j)
		{
